...
### ⚠️ Предупреждение: доверие к KWS-серверам

Трафик идёт не напрямую в Telegram, а через **сторонние релеи** (`r1.nimarko.org`, `ws.th3web.com`). Это транспортный посредник — по модели близко к публичному MTProto-прокси, а не к VPN с полной приватностью.

```
Telegram → локальный прокси (127.0.0.1) → плагин → WebSocket → KWS-релей → DC Telegram
```

**Содержимое переписки** (текст, медиа, обычные чаты) KWS **не должен** видеть: после снятия proxy-обёртки остаётся стандартный MTProto-трафик, зашифрованный сессионным ключом Telegram. Секретные чаты дополнительно защищены E2E.

**Метаданные и идентификация** — да, оператор KWS может их собирать и анализировать:

| Видит KWS | Не видит KWS |
|-----------|--------------|
| Telegram `user_id` (регистрация, заголовок `X-Cred`) | Текст сообщений |
| Объём трафика, тайминги, длительность сессий | Медиа и файлы |
| Номер DC, медиа/не-медиа канал | Auth key Telegram |
| IP, паттерны активности (когда онлайн) | Содержимое секретных чатов (E2E) |

| Угроза | Оценка |
|--------|--------|
| Прочитают переписку | 🟢 Низкая (при штатном MTProto) |
| Узнают факт использования Telegram и метаданные | 🟡 Средняя–высокая |
| Привязка к Telegram UID | 🔴 Да, при регистрации на KWS |

**Имеет смысл использовать**, если нужен обход блокировок и вы принимаете, что оператор KWS — доверенная третья сторона для метаданных. **Не подходит**, если важны анонимность или минимизация следов использования Telegram.
